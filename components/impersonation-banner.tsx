...
export function ImpersonationBanner({ user }: { user: SessionUser }) {
  ...
  return (
    <div className="bg-[var(--accent)]/10 border-b border-[var(--accent)]/30 px-4 lg:px-10 py-2 flex items-center justify-between gap-3">
      <div className="flex items-center gap-2 min-w-0">
        ...
        <p className="text-[var(--accent)] text-xs sm:text-sm font-medium truncate">
          Viewing as Manager &middot; you are the Owner
        </p>
      </div>
      <button onClick={handleSwitchBack} disabled={switching}
        className="flex-shrink-0 text-[var(--accent)] text-xs font-semibold hover:underline disabled:opacity-40">
