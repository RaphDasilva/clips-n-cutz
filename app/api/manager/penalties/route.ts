...
// Manager-facing manual penalty ledger. Operational concern;
// owner reads via /api/owner/penalties.
...
export async function GET(req: NextRequest) {
  ...
  let q = supabase
    ...
    .select('id, staff_id, amount_ngn, reason, given_at, given_by, status, reversed_at, reversed_by, created_at, users!staff_id(name)')
  ...
  // Active total per staff — what currently counts against payouts.
...
// POST — manager issues a new penalty against a staff member.
export async function POST(req: NextRequest) {
  ...
  if (!staffId || !Number.isFinite(amountNgn) || amountNgn <= 0 || !reason) {
    return NextResponse.json({ error: 'Staff, amount above ₦0 and a reason are required.' }, { status: 400 })
  }
  ...
  const { data, error } = await supabase
    ...
    .single() as { data: { id: string; staff_id: string; amount_ngn: number; reason: string; given_at: string; status: string } | null; error: { message: string } | null }

  if (error || !data) return NextResponse.json({ error: 'Failed to record penalty.' }, { status: 500 })
  return NextResponse.json({ penalty: data }, { status: 201 })
}
