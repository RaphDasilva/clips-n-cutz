...
// Staff books their own appointment. The created appointment is
// pinned to the staff member who booked it via appointments.staff_id,
// so it shows up on their dashboard and check-in defaults to them.
export async function POST(req: NextRequest) {
  const token   = req.cookies.get(SESSION_COOKIE.name)?.value
  const session = token ? await verifySessionToken(token) : null
  if (!session) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  if (session.role !== 'staff') {
    return NextResponse.json({ error: 'Only staff can book through this endpoint.' }, { status: 403 })
  }
  ...
  if (!clientName || !clientPhone || !serviceIds.length || !date || !timeValue) {
    return NextResponse.json({ error: 'Client name, phone, date, time and at least one service are required.' }, { status: 400 })
  }
