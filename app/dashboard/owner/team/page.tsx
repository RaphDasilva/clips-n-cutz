    ...
    <div className="px-6 lg:px-10 py-8 max-w-3xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-[#555] text-sm mt-0.5">Manage customer care accounts</p>
        </div>
      ...
      {loading ? (
        <div className="space-y-2">
          {[0,1].map(i => <div key={i} className="bg-[#141414] border border-[#1e1e1e] rounded-xl h-20 animate-pulse" />)}
        </div>
      ) : team.length === 0 ? (
        <div className="bg-[#141414] border border-[#1e1e1e] rounded-2xl px-4 py-16 text-center">
          <p className="text-[#555] text-sm">No manager accounts yet.</p>
          <p className="text-[#333] text-sm mt-1">Tap &quot;Add Account&quot; to create one.</p>
        </div>
      ) : (
            ...
            <div key={m.id} className="px-5 py-4 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-9 h-9 rounded-full bg-[#1e1e1e] border border-[#2a2a2a] flex items-center justify-center flex-shrink-0">
                  <span className="text-white text-sm font-semibold">{m.name.charAt(0).toUpperCase()}</span>
                </div>
                <div className="min-w-0">
                  ...
                  <p className="text-[#555] text-xs mt-0.5">{m.phone}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <button onClick={() => { setResetTarget(m); setResetPin(''); setResetError(''); setResetSuccess(false) }}
                  className="text-xs text-[#666] hover:text-white border border-[#2a2a2a] hover:border-[#3a3a3a] px-3 py-1.5 rounded-lg transition-all">
                  Reset PIN
                </button>
                <button onClick={() => toggleActive(m)}
                  className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${
                    m.is_active
                      ? 'text-[#666] hover:text-red-400 border-[#2a2a2a] hover:border-red-500/30'
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-end sm:items-center justify-center p-4"
          ...
          <div className="bg-[#0f0f0f] border border-[#2a2a2a] rounded-2xl w-full max-w-md"
            ...
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-[#1e1e1e]">
              <h2 className="text-white font-semibold">Add Manager Account</h2>
              <button onClick={() => setShowAdd(false)} className="text-[#555] hover:text-white transition-colors">
            ...
            <form onSubmit={submitAdd} className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <p className="text-[#444] text-xs mt-1.5">They will be asked to change this on first login.</p>
              </div>
              {addError && (
      ...
      {resetTarget && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-end sm:items-center justify-center p-4"
          ...
          <div className="bg-[#0f0f0f] border border-[#2a2a2a] rounded-2xl w-full max-w-md"
            onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-[#1e1e1e]">
              <h2 className="text-white font-semibold">Reset PIN — {resetTarget.name}</h2>
              <button onClick={() => setResetTarget(null)} className="text-[#555] hover:text-white transition-colors">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
                ...
                <>
                  <div>
                    ...
                    <input type="password" value={resetPin}
                      ...
                      placeholder="••••" inputMode="numeric" maxLength={4} required
                      className="input text-center tracking-[0.5em] max-w-[120px]" />
                    <p className="text-[#444] text-xs mt-1.5">They will be asked to change this on next login.</p>
                  </div>
                  {resetError && (
                    <div className="bg-red-500/5 border border-red-500/20 rounded-xl px-4 py-3">
