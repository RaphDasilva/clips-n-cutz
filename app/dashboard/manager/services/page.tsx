  ...
  return (
    <div className="px-6 lg:px-10 py-8 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-[var(--text)] text-2xl font-bold tracking-tight">Services</h1>
          <p className="text-[var(--text-dim)] text-sm mt-1">{services.filter(s => s.is_active).length} active services</p>
        </div>
        <div className="flex items-center gap-2">
          <label className="inline-flex items-center gap-2 text-[var(--text-muted)] text-xs">
          ...
          <button onClick={openCreate}
            className="bg-[var(--text)] text-[var(--bg)] font-semibold px-4 py-2 rounded-xl text-sm hover:bg-[var(--text-muted)] active:scale-[0.98] transition-all">
            + New Service
          </button>
        </div>
      </div>
      ...
      {loading ? (
        <div className="space-y-3">
          {[0,1,2].map(i => <div key={i} className="h-20 bg-[var(--card)] rounded-xl animate-pulse" />)}
        </div>
      ) : services.length === 0 ? (
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl px-6 py-12 text-center">
          <p className="text-[var(--text-muted)] text-sm">No services yet.</p>
          <button onClick={openCreate}
            className="mt-4 bg-[var(--text)] text-[var(--bg)] font-semibold px-4 py-2 rounded-lg text-sm hover:bg-[var(--text-muted)]">
            Add the first one
          </button>
        </div>
      ) : (
          ...
          {groups.map(group => (
            <div key={group.category}>
              <div className="flex items-center justify-between mb-2 px-1">
                <p className="text-[var(--text-muted)] text-[11px] font-bold uppercase tracking-wider">{group.category}</p>
                <p className="text-[var(--text-dim)] text-[10px]">{group.services.length}</p>
              </div>
                ...
                {group.services.map(s => (
                  <div key={s.id}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[var(--text)] text-sm font-medium truncate">{s.name}</p>
                      <p className="text-[var(--text-muted)] text-xs mt-0.5 tabular-nums">{fmtNaira(s.price_ngn)}</p>
                    </div>
                    {!s.is_active && (
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-[var(--text-muted)] px-2 py-0.5 rounded bg-[var(--border)]">
                        Inactive
                      </span>
                    )}
                    <button onClick={() => openEdit(s)}
                      className="text-[var(--text-muted)] text-xs font-medium hover:text-[var(--text)] transition-colors px-2 py-1 rounded hover:bg-[var(--border)]">
                      Edit
                    </button>
                    {s.is_active ? (
          ...
          <div className="bg-[var(--surface)] border border-[var(--border-strong)] w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl"
            ...
            <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-[var(--border)]">
              ...
              <button onClick={() => setModal(null)}
                className="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--text-dim)] hover:text-[var(--text)] hover:bg-[var(--border)]">
            ...
            <form onSubmit={handleSave} className="p-5 space-y-4">
              <div>
                <label className="block text-[var(--text-muted)] text-xs font-medium mb-1.5">Service Name</label>
              ...
              <div>
                ...
                <p className="text-[var(--text-dim)] text-[10px] mt-1">Pick existing or type a new category name</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[var(--text-muted)] text-xs font-medium mb-1.5">Price (₦)</label>
              ...
              <button type="submit" disabled={saving}
                className="w-full bg-[var(--text)] text-[var(--bg)] font-semibold py-3 rounded-xl text-sm hover:bg-[var(--text-muted)] disabled:opacity-40 transition-all">
      ...
      {deactivateTarget && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-[var(--surface)] border border-[var(--border-strong)] rounded-2xl max-w-sm w-full p-6"
            onClick={e => e.stopPropagation()}>
            <h2 className="text-[var(--text)] font-semibold">Deactivate &quot;{deactivateTarget.name}&quot;?</h2>
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={() => setDeactivateTarget(null)} disabled={deactivating}
                className="flex-1 bg-[var(--elevated)] border border-[var(--border-strong)] text-[var(--text)] font-medium py-2.5 rounded-xl text-sm hover:bg-[var(--border-strong)]">
                Cancel
              </button>
